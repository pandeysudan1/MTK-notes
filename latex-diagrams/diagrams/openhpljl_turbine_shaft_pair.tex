...
\tikzset{
  box/.style={draw,rectangle,rounded corners=2pt,minimum width=64mm,minimum height=42mm,align=left},
...
\node[box,right=of turb] (shaft) {
\textbf{LumpedShaft}\\[1mm]
state: $\omega$\\
\[
J\dot\omega=
\tau_{\rm drive}+\tau_{\rm load}
-d(\omega-\omega_0)
\]
$\omega_{\rm drive}=\omega_{\rm load}=\omega$
};

\node[box,right=of shaft] (gen) {
\textbf{SMIBGenerator}\\[1mm]
state: $\delta$\\
