...
\tikzset{
  box/.style={draw,rectangle,rounded corners=2pt,minimum width=70mm,minimum height=38mm,align=left},
...
\node[box] (physics) {
\textbf{OpenHPLjl RigidPipe: model}\\[1mm]
State: $Q$\\
...
\node[box,right=of physics] (code) {
\textbf{ModelingToolkit implementation}\\[1mm]
...
\node[box,below=of physics] (surge) {
\textbf{OpenHPLjl SurgeTank: model}\\[1mm]
State: $H$\\
\[
A_s\dot H=Q_{\rm inlet}+Q_{\rm outlet}
\]
\[
H_{\rm inlet}=H_{\rm outlet}=H
\]
};

\node[box,right=of surge] (surgecode) {
\textbf{ModelingToolkit implementation}\\[1mm]
